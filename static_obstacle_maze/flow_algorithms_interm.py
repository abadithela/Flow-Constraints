# Computing max flow without passing through intermediate node
import sys
sys.path.append('..')
import numpy as np
...
def find_max_flow_pyomo(maze):
    ...
    nodes = list(G.nodes())
    for k in nodes:
        if maze.map[k[0], k[1]] == 'o': # remove obstacles
            G.remove_node(k)
...
def get_max_flow(maze):
    ...
    for i,j in edges:
        if i == j:
            G.remove_edge(i,j)
        if maze.map[i[0],i[1]] == 'o' or maze.map[j[0],j[1]]=='o': # remove obstacle:
            G.remove_edge(i,j)
