...
# Both pyomo and gradient descent versions
...
# Gradient function depends on g in the inner argument.
def gradient_dep(xt, yt, lamt, c1, edges_keys, LAMBDA):
    nx = len(xt)
    ne = len(list(edges_keys.keys()))
    gradf = np.multiply(c1,np.ones((nx,1)))# Column vector
    # The dependent constraint is: f3e + de <= 1
    gradg = dict()
    for i in range(ne):
        gradg[i] = np.zeros((3*ne+1,1))
        gradg[i][-1] = 1 # Derivative of t wrt to t is 1
        de_idx = 2*ne+i # Index of the de corresponding to this constraint
        gradg[i][de_idx] = -1
    grad_lag = 0.0
    for i in range(ne):
        grad_lag += gradg[i]*lamt[i]
    grad = gradf + grad_lag
    return grad
...
def gradient(xt, yt, lamt, c1, c2, Aineq, bineq, Aeq, beq):
    ...
    gradg = np.vstack((Aineq[:, :len(xt)], Aeq[:, :len(xt)])) # The columns of A corresponding to x
...
def max_oracle_pyomo(T, x0, eta, c1, c2, Aeq, beq, Aineq, bineq, Aeq_proj, beq_proj, Aineq_proj, bineq_proj, eq_cons_names, ineq_cons_names, edges_keys,nodes_keys, src, sink, int, maze=None):
    '''
    First-order gradient descent optimization with Pyomo being the oracle for max flow. Similar to above except that the Lagrangian for equality constraints corresponding to this function is set to lam[c].
    '''
    LAMBDA = 1
    EPS_FLG = 1e-6
    grad_norm = []
...
def find_edge_key(edges_keys, edge):
    for kedge, edge_val in edges_keys.items():
        if edge == edge_val:
            return kedge
    pdb.set_trace()

def match_dual_vars_dep(lamt, edges_keys):
    ne = len(list(edges_keys.keys()))
    dual_vars = {i: 0.0 for i in range(ne)}
    for edge_key, edge in edges_keys.items():
        edge_list_form = "["+str(edge[0][0])+ ","+str(edge[0][1]) +"," +  str(edge[1][0]) + "," + str(edge[1][1]) + "]"
        lamt_key = "cut_cons3"+edge_list_form
        dual_vars[edge_key] = lamt[lamt_key]
    return dual_vars
...
def max_oracle_pyomo_v2(T, x0, eta, c1, c2, Aeq, beq, Aineq, bineq, Aeq_proj, beq_proj, Aineq_proj, bineq_proj, eq_cons_names, ineq_cons_names, edges_keys,nodes_keys, src, sink, int, maze=None):
    '''
    First-order gradient descent optimization with Pyomo being the oracle for max flow. Similar to above except that the Lagrangian for equality constraints corresponding to this function is set to zero.
    '''
    LAMBDA = 1e4
    EPS_FLG = 1e-6
    fval = []
...
def max_oracle_pyomo_dep(T, x0, eta, c1, c2, Aeq, beq, Aineq, bineq, Aeq_proj, beq_proj, Aineq_proj, bineq_proj, eq_cons_names, ineq_cons_names, edges_keys,nodes_keys, src, sink, int, maze=None):
    ...
    for t in range(1,T-1):
        yt, lamt, fv = Vin_oracle(edges_keys, nodes_keys, src, sink, int, xtraj[t-1], LAMBDA)
        dual_vars = match_dual_vars_dep(lamt, edges_keys)
        ytraj[t-1] = yt.copy()
        xstep = xtraj[t-1] - 0.5/t*gradient_dep(xtraj[t-1], ytraj[t-1], dual_vars, c1, edges_keys, LAMBDA)
        fval.append(fv)
        if proj == "des":
            xtraj[t] = projx_des(xstep, xtraj[t-1], ne, Aineq_proj, bineq_proj, Aeq_proj, beq_proj)
        else:
...
def Vin_oracle(edges_keys, nodes_keys, src, sink, int, x,LAMBDA):
    ...
    yt, lamt, fv = max_flow_oracle_debug(edges_keys, nodes_keys, src, sink, int, x,LAMBDA)
    return yt, lamt, fv
