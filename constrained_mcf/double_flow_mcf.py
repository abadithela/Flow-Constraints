...
class MCF():
    ...
    def check_flows(self, node):
        f1_e, f2_e, F, d_e = self.variables
        outgoing = []
        incoming = []
        ...
        return incoming, outgoing, flow_1, flow_2
...
if __name__ == '__main__':
    ...
    source_dict = {'s1': (5,0), 's2': (2,2)}
    sink_dict = {'t1': (2,2), 't2': (0,9)}
    ...
    f1_e, f2_e, F, d_e = mcf_problem.compute_sol(model)
    opt_flows = [f1_e, f2_e, F, d_e]
    mcf_problem.plot_flow(opt_flows=opt_flows)
    flow_c1, flow_c2, cuts = mcf_problem.process_sol(opt_flows=opt_flows)
