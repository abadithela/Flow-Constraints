...
class MCF():
    ...
    def add_vars(self, model):
        ...
        for (i,j) in self.graph.edges():
            ...
            count += 1
        self.variables = [model.f1_e, model.F, model.d_e]
...
if __name__ == '__main__':
    ...
    f1_e, F, d_e = mcf_problem.compute_sol(model)
    opt_flows = [f1_e, F, d_e]
    mcf_problem.plot_flow(opt_flows=opt_flows)

    # Solve for max flow:
    print("Solution constructed!")
    incoming_s1, outgoing_s1, flow1_s1 = mcf_problem.check_flows(source_dict['s1'])
    incoming_t1, outgoing_t1, flow1_t1 = mcf_problem.check_flows(sink_dict['t1'])
    pdb.set_trace()
    print("Read solution")
    # Save LP solution:
